...
summarise_ranked_models_mort <- function(rankedModels, cores = 1, eps = .Machine$double.eps*100) {
  #sums <- mclapply(rankedModels$ModelFit, get_model_detail, mc.cores = cores) NOT SUPPORTED ON WINDOWS
...
get_model_detail_mort <- function(formula, eps) {
  model <- gam(as.formula(formula),
               family = betar(link = "logit", eps = eps),
               data = pathogen_data,
  ...
  dev <- get_relative_deviance_mort(model, modelSummary$formula, eps = eps) %>% 
    select(-Model) %>% 
...
get_relative_deviance_mort <- function(model, formula = NULL, eps = .Machine$double.eps*100) {
  # if (! model$method %in% c('REML', 'ML')) stop('Model not fit using ML/REML')  # Not sure method below is valid for GCV
  # 
  ...
  # Get null deviance:
  if (startsWith(fullModel$family$family, "Beta regression")) {
    # Fix theta estimate to that from the full model
    theta <- fullModel$family$getTheta(trans = TRUE)
    link <- fullModel$family$link
    
    fixed_family <- betar(link = link, theta = theta, eps = eps)
  } else {
    fixed_family <- fullModel$call$family
  }
  
  nullModel <- update(fullModel, formula. = . ~ 1, family = fixed_family)
  nullDeviance <- deviance(nullModel)
